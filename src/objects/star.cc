...
Model Star::_sphere_model;

Star::Star(PlanetData const &pd)
  : Planet(pd) 
//    radius(pd.radius),
...
//    FLOG("Star constructor, address %p, texture %u, texture file %s\n", this, _texture, pd.texture_file);
}


#if 0
Star::Star(Float radius_,
...
Star::Star(Star &&rvalue) :
    Planet(std::move(rvalue))
...
//    FLOG("Star move constructor, from %p to %p, texture %u\n", &rvalue, this, _texture);
...
//        return *this;
    ...
    Planet::operator=(std::move(static_cast<Planet&&>(p)));
//    radius = p.radius;
...
//    FLOG("Star move assignment, from %p to %p, texture %u\n", &p, this, _texture);
...
mat4 Star::render(mat4 matrix) const {
//    glPushMatrix();
//
...
//    if (!is_moon) // When rendering moons we are already in star's orbital plane
...
//    glMultMatrixf(&model_matrix[0][0]);
    ...
    auto return_matrix = matrix * crr_model_matrix;
    for (auto it = moons.begin(); it != moons.end(); it++)
        it->render(return_matrix);
...
//    glDrawElements(GL_TRIANGLES, indices.size(), INDEX_TYPE, (void*) 0);
    ...
    //    GLUquadricObj *star = gluNewQuadric();
//    gluQuadricDrawStyle(star, GLU_FILL);
//    gluQuadricTexture(star, GLU_TRUE);
//    gluQuadricNormals(star, GLU_SMOOTH);
//
//    gluSphere(star, radius, 50, 50);
//
...
mat4 Star::render_orbit(mat4 matrix) const
{
    matrix *= _model_matrix;
