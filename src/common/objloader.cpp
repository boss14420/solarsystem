...
bool loadOBJ(
	...
	for( unsigned int i=0; i<vertexIndices.size(); i++ ){
		...
		vec3data vertex = static_cast<vec3data>(temp_vertices[ vertexIndex-1 ]);
		vec2data uv = static_cast<vec2data>(temp_uvs[ uvIndex-1 ]);
		vec3data normal = static_cast<vec3data>(temp_normals[ normalIndex-1 ]);
